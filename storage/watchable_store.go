...
func (s *watchableStore) syncWatchings() {
	...
	fmt.Println(len(watchingToEvents))
	for w, evs := range watchingToEvents {
		select {
		case w.ch <- evs:
...
func (s *watchableStore) notify(rev int64, evs []storagepb.Event) {
	...
	for w, evs := range watchingToEvents {
		...
		default:
			...
			// will be processed next time and hopefully it will not be full.
			for _, ev := range evs {
				wm, ok := s.synced[string(ev.Kv.Key)]
				if !ok {
					continue
				}
				for wc := range wm {
					wc.cur = rev
					s.unsynced[wc] = struct{}{}
					delete(wm, wc)
				}
				slowWatchingGauge.Add(float64(len(wm)))
			}
		}
	}
}
