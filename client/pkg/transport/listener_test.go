...
func TestServerConfig_ReloadTrustedCA_WithAllowedCN(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Create server TLSInfo with ReloadTrustedCA and AllowedCN
			tlsInfo := TLSInfo{
				...
				CAReloadInterval: 100 * time.Millisecond,
				AllowedCN:        "valid-client",
				Logger:           zaptest.NewLogger(t),
			}
			tlsInfo.EnableCAReloaderTracking()
			t.Cleanup(func() { tlsInfo.Close() })
...
func TestServerConfig_ReloadTrustedCA_WithAllowedCN_DynamicReload(t *testing.T) {
	...
	tlsInfo := TLSInfo{
		...
		ClientCertAuth:   true,
		ReloadTrustedCA:  true,
		CAReloadInterval: 50 * time.Millisecond, // Fast reload for testing
		AllowedCN:        "valid-client",
		Logger:           zaptest.NewLogger(t),
	}
	tlsInfo.EnableCAReloaderTracking()
