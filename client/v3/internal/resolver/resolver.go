...
func (r *EtcdManualResolver) Build(target resolver.Target, cc resolver.ClientConn, opts resolver.BuildOptions) (resolver.Resolver, error) {
	...
	// Seed the underlying manual resolver with the complete state (endpoints +
	// ServiceConfig) before building. This makes Build emit a single resolver
	// update instead of two: previously Build sent an update without the
	// ServiceConfig, immediately followed by a second update carrying it, which
	// forced gRPC to switch balancers mid-connection and tear down an in-flight
	// SubConn (producing spurious "operation was canceled" dial warnings).
