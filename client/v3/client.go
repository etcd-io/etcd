...
func (c *Client) dial(creds grpccredentials.TransportCredentials, dopts ...grpc.DialOption) (*grpc.ClientConn, error) {
	...
	if dialTo := c.cfg.DialTimeout; dialTo > 0 {
		var cancel context.CancelFunc
		dctx, cancel := context.WithTimeout(c.ctx, dialTo)
		defer cancel()
...
func waitForConnection(ctx context.Context, conn *grpc.ClientConn) error {
	...
	//
	// The downside is reduced error clarity: we may surface context deadline
	// exceeded instead of the underlying cause (for example, connection refused
	// because the endpoint is not ready, or an x509 error from TLS misconfiguration).
	// Users need to enable --debug mode to see the original error.
