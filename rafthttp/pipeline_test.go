...
func TestPipelineSend(t *testing.T) {
	...
	p := newPipeline(tr, "http://10.0.0.1", types.ID(2), types.ID(1), types.ID(1), fs, &fakeRaft{}, nil)
...
func TestPipelineSendFailed(t *testing.T) {
	fs := &stats.FollowerStats{}
	p := newPipeline(newRespRoundTripper(0, errors.New("blah")), "http://10.0.0.1", types.ID(2), types.ID(1), types.ID(1), fs, &fakeRaft{}, nil)
...
func TestPipelinePost(t *testing.T) {
	tr := &roundTripperRecorder{}
	p := newPipeline(tr, "http://10.0.0.1", types.ID(2), types.ID(1), types.ID(1), nil, &fakeRaft{}, nil)
...
func TestPipelinePostBad(t *testing.T) {
	...
	for i, tt := range tests {
		p := newPipeline(newRespRoundTripper(tt.code, tt.err), tt.u, types.ID(2), types.ID(1), types.ID(1), nil, &fakeRaft{}, make(chan error))
...
func TestPipelinePostErrorc(t *testing.T) {
	...
	for i, tt := range tests {
		...
		p := newPipeline(newRespRoundTripper(tt.code, tt.err), tt.u, types.ID(2), types.ID(1), types.ID(1), nil, &fakeRaft{}, errorc)
