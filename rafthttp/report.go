...
func reportOpError(local, remote types.ID, t string, err *opError) {
	linkErrMapMu.Lock()
	defer linkErrMapMu.Unlock()
	name := linkName{local: local, remote: remote, t: t}
	now := time.Now()
	if !err.equal(linkErrMap[name].err) || now.Sub(linkErrMap[name].last) > errBackoffTime {
		linkErrMap[name] = linkErr{err: err, last: now}
		log.Printf("rafthttp: failed to %s %s in %s: %v", err.op, remote, t, err)
	}
}
