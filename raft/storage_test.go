...
func TestStorageApplySnapshot(t *testing.T) {
	...
	tests := []struct {
		...
		{pb.Snapshot{Data: data, Metadata: pb.SnapshotMetadata{Index: 4, Term: 4, ConfState: *cs}}},
		{pb.Snapshot{Data: data, Metadata: pb.SnapshotMetadata{Index: 3, Term: 3, ConfState: *cs}}},
	}

	s := NewMemoryStorage()

	//Apply Snapshot successfull
	i := 0
	tt := tests[i]
	err := s.ApplySnapshot(tt.wsnap)
	if err != nil {
