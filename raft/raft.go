...
func newRaft(c *Config) *raft {
	...
	for _, p := range peers {
		r.prs[p] = &Progress{Next: 1, ins: newInflights(r.MaxInflightMsgs)}
	}
...
func (r *raft) sendAppend(to uint64) {
	...
	ents, erre := r.raftLog.entries(pr.Next, r.MaxSizePerMsg)
...
// tickElection is run by followers and candidates after r.ElectionTick.
...
// tickHeartbeat is run by leaders to send a MsgBeat after r.HeartbeatTick.
...
func (r *raft) Step(m pb.Message) error {
	...
	case m.Term > r.Term:
		...
		if m.Type == pb.MsgVote || m.Type == pb.MsgPreVote {
			...
			inLease := r.CheckQuorum && r.lead != None && r.electionElapsed < r.ElectionTick
			if !force && inLease {
				// If a server receives a RequestVote request within the minimum election timeout
				// of hearing from a current leader, it does not update its term or grant its vote
				r.Logger.Infof("%x [logterm: %d, index: %d, vote: %x] ignored %s from %x [logterm: %d, index: %d] at term %d: lease is not expired (remaining ticks: %d)",
					r.ID, r.raftLog.lastTerm(), r.raftLog.lastIndex(), r.Vote, m.Type, m.From, m.LogTerm, m.Index, r.Term, r.ElectionTick-r.electionElapsed)
				return nil
			}
			lead = None
		}
...
func (r *raft) setProgress(id, match, next uint64) {
	r.prs[id] = &Progress{Next: next, Match: match, ins: newInflights(r.MaxInflightMsgs)}
}
