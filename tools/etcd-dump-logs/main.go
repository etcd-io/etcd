...
func main() {
	...
	startIndex := flag.Uint64("start-index", 0, "The index to start dumping")
	endIndex := flag.Uint64("end-index", math.MaxUint64, "The index to stop dumping")
...
func readUsingReadAll(lg *zap.Logger, startFromIndex bool, startIndex *uint64, endIndex *uint64, snapfile *string, dataDir string, waldir *string) []raftpb.Entry {
	...
	if err != nil && (!startFromIndex || !errors.Is(err, wal.ErrSnapshotNotFound)) {
		// ignore ErrSliceOutOfRange if just requesting a specific range of entries
