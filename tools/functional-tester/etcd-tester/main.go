...
func main() {
	...
	qps := flag.Int("qps", 2000, "stresser requests per second. If negative, stress without any limits.")
	...
	c, err := newCluster(endpoints, *datadir, *stressKeySize, *stressKeySuffixRange, *qps, *isV2Only)
	if err != nil {
		plog.Fatal(err)
	}
	defer c.Terminate()
