...
func (s *loggingPayloadSuite) TestPingError_LogsError() {
	...
	s.assertField("post.response.msg.error", "rpc error: code = DeadlineExceeded desc = Userspace error", 1)
}
...
func (s *loggingPayloadSuite) TestPingStream_LogsAllRequestsAndResponses() {
	...
	s.Require().NoError(waitUntil(200*time.Millisecond, ctx.Done(), func() error {
		if len(s.logs.FilterFieldKey("post.request.msg.error").All()) > 0 {
			return nil
		}
		return fmt.Errorf("no EOF log yet")
	}))
	eof := s.logs.FilterFieldKey("post.request.msg.error").All()
	s.Len(eof, 1)
	s.Equal(io.EOF.Error(), eof[0].ContextMap()["post.request.msg.error"])
