...
// proxylogger implements the go-grpc-middleware v2 Reporter interface
...
func (r *proxylogger) PostCall(_ error, _ time.Duration) {
	// no op - no post-call payload logging
}

func (r *proxylogger) PostMsgReceive(payload any, err error, _ time.Duration) {
...
func (r *proxylogger) PostMsgSend(payload any, err error, _ time.Duration) {
...
func logFieldsFromPayload(payload any, err error, callType string) []zap.Field {
	fields := []zap.Field{}
	if err != nil {
		fields = append(fields, zap.NamedError(fmt.Sprintf("post.%s.msg.error", callType), err))
	}
