	...
	for _, tc := range tcs {
		t.Run(tc.name, func(t *testing.T) {
			...
			if l.getRemainingTTL() != ttl {
				t.Errorf("remainingTTL() = %d, expected: %d", l.getRemainingTTL(), ttl)
			}
			le.Checkpoint(2, checkpointTTL)
			if l.getRemainingTTL() != checkpointTTL {
				t.Errorf("remainingTTL() = %d, expected: %d", l.getRemainingTTL(), checkpointTTL)
			}
			le.Stop()
			le2 := newLessor(lg, be, clusterLatest(), cfg)
			l = le2.Lookup(2)
			if l.getRemainingTTL() != tc.expectRemainingTTL {
				t.Errorf("remainingTTL() = %d, expected: %d", l.getRemainingTTL(), tc.expectRemainingTTL)
			}
		})
	}
}
