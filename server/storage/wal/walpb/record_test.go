...
func TestValidateSnapshot(t *testing.T) {
	tests := []struct {
		...
		{name: "invalid", snap: &Snapshot{Index: ptr(uint64(5)), Term: ptr(uint64(3))}, wantErr: true},
		{name: "valid", snap: &Snapshot{Index: ptr(uint64(5)), Term: ptr(uint64(3)), ConfState: &raftpb.ConfState{Voters: []uint64{0x00cad1}}}, wantErr: false},
	}
