...
func TestOpenAtIndex(t *testing.T) {
	...
	w, err := Open(zaptest.NewLogger(t), dir, 0)
	require.NoErrorf(t, err, "err = %v, want nil", err)
	if g := filepath.Base(w.tail().Name()); g != walName(0, 0) {
		t.Errorf("name = %+v, want %+v", g, walName(0, 0))
	}
	...
	w, err = Open(zaptest.NewLogger(t), dir, 5)
	require.NoErrorf(t, err, "err = %v, want nil", err)
	...
	emptydir := t.TempDir()
	if _, err = Open(zaptest.NewLogger(t), emptydir, 0); !errors.Is(err, ErrFileNotFound) {
...
func TestVerify(t *testing.T) {
	...
	hardstate, err := Verify(lg, walDir, 0)
	if err != nil {
...
func TestSaveWithCut(t *testing.T) {
	...
	neww, err := Open(zaptest.NewLogger(t), p, 0)
	require.NoErrorf(t, err, "err = %v, want nil", err)
	defer neww.Close()
...
func TestRecover(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if w, err = Open(zaptest.NewLogger(t), p, 0); err != nil {
				t.Fatal(err)
			}
...
func TestRecoverAfterCut(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		w, err := Open(zaptest.NewLogger(t), p, uint64(i))
		if err != nil {
			if i <= 4 {
...
func TestOpenAtUncommittedIndex(t *testing.T) {
	...
	w, err = Open(zaptest.NewLogger(t), p, 0)
	if err != nil {
		t.Fatal(err)
	}
...
func TestOpenForRead(t *testing.T) {
	...
	w2, err := OpenForRead(zaptest.NewLogger(t), p, 0)
	if err != nil {
		t.Fatal(err)
	}
	defer w2.Close()
...
func TestOpenWithMaxIndex(t *testing.T) {
	...
	w2, err := Open(zaptest.NewLogger(t), p, 0)
	if err != nil {
		t.Fatal(err)
	}
	defer w2.Close()
...
func TestRestartCreateWal(t *testing.T) {
	...
	if w, err = OpenForRead(zaptest.NewLogger(t), p, 0); err != nil {
		t.Fatal(err)
	}
	defer w.Close()
...
func TestOpenOnTornWrite(t *testing.T) {
	...
	w, err = OpenForRead(zaptest.NewLogger(t), p, 0)
	if err != nil {
		t.Fatal(err)
	}
...
func TestValidSnapshotEntriesAfterPurgeWal(t *testing.T) {
	...
	files, _, err := selectWALFiles(nil, p, 0)
	if err != nil {
		t.Fatal(err)
	}
	os.Remove(p + "/" + files[0])
...
func TestLastRecordLengthExceedFileEnd(t *testing.T) {
	...
	w, err := Open(zaptest.NewLogger(t), filepath.Dir(fileName), 0)
	require.NoError(t, err)
	defer w.Close()
