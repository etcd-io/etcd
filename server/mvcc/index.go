...
// Put 一是向 BTree 中添加 keyIndex实例， 二是 向 keyIndex 中追加 revision 信息
func (ti *treeIndex) Put(key []byte, rev revision) {
	...
	if item == nil {
		...
		ti.tree.ReplaceOrInsert(keyi) // 想BTree 添加元素并返回 nil，如果待添加的元素己存在，则将其删除并返回。
		return
	}
	okeyi := item.(*keyIndex)
...
// Range BTree 中 key～end之间元素中指定的 revision信息
func (ti *treeIndex) Range(key, end []byte, atRev int64) (keys [][]byte, revs []revision) {
	if end == nil {
		// 如果未指定 end 参数，只查询 key 对应的 revision 信息
		rev, _, _, err := ti.Get(key, atRev)
		if err != nil {
			return nil, nil
		}
