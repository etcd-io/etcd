...
// 把当前txWriteBuffer中键值对合并到指定的txReadBuffer实例中，达到了更新只读事务缓存的效果
...
func (bb *bucketBuffer) Range(key, endKey []byte, limit int64) (keys [][]byte, vals [][]byte) {
	...
	// 从前面查找到的 idx位置开始，直到 endKey或是 遍历的键值对个数 达到 limit上限为止
...
// 将传入的 bbsrc(bucketBuffer实例)与当前的 bucketBuffer 进行合井 ， 之后会对合并结果进行排序和去重，具体实现如下所示 。
