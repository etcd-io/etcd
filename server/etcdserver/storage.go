...
type Storage interface {
	...
	// Save()方法负责将 Entry记录和 HardState状态信息保存到底层的持久化存储上，该方法可能会阻塞，
	// 在后面的介绍中会看到， Storage 接口 的实现是通过 WAL 模块将上述数据持久化到 WAL 日志文件中的
	Save(st raftpb.HardState, ents []raftpb.Entry) error
	// SaveSnap function saves snapshot to the underlying stable storage.
	// SaveSnap()方法负责将快照数据持久到底层的持久化存储上，该方法也可能会阻塞，在后面分析中会看到，
	// Storage 接口的实现是使用前文介绍的 Snapshotter 将快照数据保存到快照文件中的
