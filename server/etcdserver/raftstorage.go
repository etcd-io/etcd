...
func (ms *MemoryStorage) ApplyConfState(index, term uint64, confstate pb.ConfState, ents []pb.Entry) error {
	ms.Lock()
	defer ms.Unlock()

	ms.snapshot.Metadata.Index = index
	ms.snapshot.Metadata.Term = term
	ms.snapshot.Metadata.ConfState = confstate
	for i := 0; i < len(ents); i++ {
		if ents[i].Index >= index && ents[i].Term >= term {
			ms.ents = ents[i:]
			break
		}
	}
...
func (ms *MemoryStorage) CreateSnapshot(i uint64, cs *pb.ConfState, data []byte) (pb.Snapshot, error) {
	...
	offset := ms.ents[0].Index
	if i > ms.lastIndex() {
		panic(fmt.Sprintf("snapshot %d is out of bound lastindex(%d)", i, ms.lastIndex()))
	}
