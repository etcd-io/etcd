...
func put(ctx context.Context, txnWrite mvcc.TxnWrite, lessor lease.Lessor, req *pb.PutRequest) (resp *pb.PutResponse, err error) {
	...
	val, prevKV, leaseID, err := checkPut(txnWrite, lessor, req)
	if err != nil {
		return nil, err
	}
	resp.PrevKv = prevKV
...
func checkPut(rv mvcc.ReadView, lessor lease.Lessor, req *pb.PutRequest) (val []byte, prevKV *mvccpb.KeyValue, leaseID lease.LeaseID, err error) {
	val, leaseID = req.Value, lease.LeaseID(req.Lease)

	if req.IgnoreValue || req.IgnoreLease || req.PrevKv {
		resp, err := rv.Range(context.TODO(), req.Key, nil, mvcc.RangeOptions{})
		if err != nil {
		...
		if (req.IgnoreValue || req.IgnoreLease) && prevKV == nil {
			...
			return nil, nil, 0, errors.ErrKeyNotFound
		}
	}
	if req.IgnoreValue {
		val = prevKV.Value
...
func checkTxn(rv mvcc.ReadView, rt *pb.TxnRequest, lessor lease.Lessor, txnPath []bool) (int, error) {
	...
	for _, req := range reqs {
		...
		case *pb.RequestOp_RequestPut:
			_, _, _, err = checkPut(rv, lessor, tv.RequestPut)
		case *pb.RequestOp_RequestDeleteRange:
		case *pb.RequestOp_RequestTxn:
