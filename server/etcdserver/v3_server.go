...
func (s *EtcdServer) linearizableReadLoop() {
	for {
		...
		confirmedIndex, responseID, err := s.requestCurrentIndex(leaderChangedNotifier)
		if isStopped(err) {
			return
		}
		if err != nil {
		...
		s.Logger().Info("linearizableReadLoop: confirmed request", zap.Uint64("read-request-id", responseID), zap.Uint64("confirmed-index", confirmedIndex), zap.Uint64("server-term", s.getTerm()), zap.Uint64("server-applied-index", s.getAppliedIndex()), zap.Uint64("server-committed-index", s.getCommittedIndex()))
		nr.notify(nil, confirmedIndex, responseID)
...
func (s *EtcdServer) requestCurrentIndex(leaderChangedNotifier <-chan struct{}) (uint64, uint64, error) {
	...
	for {
		...
		case rs := <-s.r.readStateC:
			...
			case <-leaderChangedNotifier:
				...
				return 0, 0, errors.ErrLeaderChanged
			default:
			}
			responseID := uint64(0)
			...
			return rs.Index, responseID, nil
		case <-leaderChangedNotifier:
			readIndexFailed.Inc()
			lg.Warn("requestCurrentIndex: leader changed, retrying", zap.Uint64("read-request-id", requestID))
