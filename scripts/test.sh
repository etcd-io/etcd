...
function bom_pass {
  ...
  # Procfs is not used in darwin (osx) builds, thus we need to include it manually in BOM check. See https://github.com/etcd-io/etcd/pull/19599 for more details.
  if [ "$OS" = "darwin" ] ; then
    output=$(echo "$output" | jq --tab '. + [{
