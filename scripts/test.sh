...
function integration_pass {
  local exit_code=0
  run_for_module "tests" go_test "./integration/..." "parallel" : -timeout="${TIMEOUT:-15m}" "${COMMON_TEST_FLAGS[@]}" "${RUN_ARG[@]}" "$@" || { echo "integration suite failed with ./integration/... package" && exit_code=1; }
  run_for_module "tests" go_test "./common/..." "parallel" : --tags=integration -timeout="${TIMEOUT:-15m}" "${COMMON_TEST_FLAGS[@]}" "${RUN_ARG[@]}" "$@" || { echo "integration suite failed with ./common/... package" && exit_code=1; }
  integration_extra "$@" || { echo "integration_extra suite failed" && exit_code=1; }
...
function e2e_pass {
  ...
  local exit_code=0
  run_for_module "tests" go_test "./e2e/..." "keep_going" : -timeout="${TIMEOUT:-30m}" "${RUN_ARG[@]}" "$@" || { echo "e2e suite failed with ./e2e/... package" && exit_code=1; }
  run_for_module "tests" go_test "./common/..." "keep_going" : --tags=e2e -timeout="${TIMEOUT:-30m}" "${RUN_ARG[@]}" "$@" || { echo "e2e suite failed with ./common/... package" && exit_code=1; }
