...
function cov_pass {
  ...
  run_go_tests "${modules[@]}" \
               -short \
               -timeout=30m \
               "${gocov_build_flags[@]}" \
               -coverprofile="${coverdir}/unit.coverprofile" \
               "$@" || failed+=("unit")
  ...
  run_go_tests "./tests/integration/..." \
               -timeout=30m \
               "${gocov_build_flags[@]}" \
               -coverprofile="${coverdir}/integration.coverprofile" \
               "$@" || failed+=("integration")
  # integration-store-v2
  run_go_tests "./tests/integration/v2store/..." \
               -timeout=5m \
               "${gocov_build_flags[@]}" \
               -coverprofile="${coverdir}/integration_v2.coverprofile" \
               "$@" || failed+=("integration_v2")
  # integration_cluster_proxy
  run_go_tests "./tests/integration/..." \
               -tags cluster_proxy \
               -timeout=30m \
               "${gocov_build_flags[@]}" \
               -coverprofile="${coverdir}/integration_cluster_proxy.coverprofile" \
               "$@" || failed+=("integration_cluster_proxy")
