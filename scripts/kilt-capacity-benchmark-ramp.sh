...
RUN_GROUP="${RUN_GROUP:-kilt-capacity}"
...
WATCH_STREAMS="${WATCH_STREAMS:-5}"
WATCHERS_PER_STREAM="${WATCHERS_PER_STREAM:-10}"
WATCH_KEY_SIZE="${WATCH_KEY_SIZE:-64}"
WATCH_VAL_SIZE="${WATCH_VAL_SIZE:-512}"
MIXED_RANGE_RATE_DIVISOR="${MIXED_RANGE_RATE_DIVISOR:-10}"
MIXED_RANGE_KEY_TOTAL="${MIXED_RANGE_KEY_TOTAL:-10000}"
MIXED_RANGE_LIMIT="${MIXED_RANGE_LIMIT:-100}"
MIXED_RANGE_CONSISTENCY="${MIXED_RANGE_CONSISTENCY:-s}"
RANGE_LIMIT="${RANGE_LIMIT:-100}"
RANGE_CONSISTENCY="${RANGE_CONSISTENCY:-s}"

# step|rate|total|clients|conns|key-space-size
PUT_STEPS=(
	"rate-500|500|150000|64|16|100000"
	"rate-1000|1000|300000|128|32|200000"
	"rate-2000|2000|600000|256|64|400000"
	"rate-4000|4000|1200000|512|128|800000"
	"rate-8000|8000|2400000|768|192|1600000"
	"rate-16000|16000|4800000|1024|256|3200000"
)

# Keep txn keyspace bounded because txn-put seeds all keys before measuring CAS.
TXN_STEPS=(
	"rate-500|500|150000|64|16|10000"
	"rate-1000|1000|300000|128|32|20000"
	"rate-2000|2000|600000|256|64|40000"
	"rate-4000|4000|1200000|512|128|80000"
	"rate-8000|8000|2400000|768|192|100000"
	"rate-16000|16000|4800000|1024|256|100000"
)

# Watch fan-out grows as writes * watchers, so these defaults are intentionally capped.
WATCH_STEPS=(
	"rate-500|500|150000|64|16|100000"
	"rate-1000|1000|300000|128|32|200000"
	"rate-2000|2000|600000|256|64|400000"
...
RANGE_STEPS=(
	"rate-500|500|150000|64|16|0"
	"rate-1000|1000|300000|128|32|0"
	"rate-2000|2000|600000|256|64|0"
	"rate-4000|4000|1200000|512|128|0"
	"rate-8000|8000|2400000|768|192|0"
)
...
Interactive KILT etcd capacity ramp. It pauses before each step and streams
benchmark output live while saving per-step logs, JSON metrics, HTML elbow
graphs, and Markdown summary tables.

Default scenarios:
  put            Sustained CAS TwoPcRound write throughput.
  txn-put        CAS Txn(Compare(ModRevision)=X, Then(Put)).
  watch-latency  Watch fan-out while CAS writes are happening.
  mixed          CAS writes + watch fan-out + concurrent range reads.
  range          Pure range/read load over existing keys.
	...
	case "${scenario}" in
		...
		txn-put)
			cmd_args+=(
				...
				--key-space-size="${key_space_size}"
				--txn-ops="${TXN_OPS}"
				--key-size="${TXN_KEY_SIZE}"
				--val-size="${TXN_VAL_SIZE}"
			)
			;;
		watch-latency)
			cmd_args+=(
				...
				--clients="${clients}"
				--conns="${conns}"
				--streams="${WATCH_STREAMS}"
				--watchers-per-stream="${WATCHERS_PER_STREAM}"
				--key-size="${WATCH_KEY_SIZE}"
				--val-size="${WATCH_VAL_SIZE}"
			)
			;;
		mixed)
			...
			cmd_args+=(
				...
				--range-rate="${range_rate}"
				--range-key-total="${MIXED_RANGE_KEY_TOTAL}"
				--range-limit="${MIXED_RANGE_LIMIT}"
				--range-consistency="${MIXED_RANGE_CONSISTENCY}"
			)
			;;
		range)
			cmd_args+=(
...
echo "KILT capacity benchmark ramp"
...
echo "All requested capacity ramp steps completed."
