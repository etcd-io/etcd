...
if [ -z "$VER" ]; then
  VER=$(git describe --always --dirty --tags --match='v*' --abbrev=14 || echo "GitNotFound")
fi
    ...
    for TARGET_ARCH in "${TARGET_ARCHS[@]}"; do
      ...
      if [ ${GOOS} == "linux" ]; then
        ...
        GZIP=-n ${tarcmd} --sort=name \
          ...
          -zcf "${TARGET}.tar.gz" -C release "${NAME}"
        echo "Wrote ${TARGET}.tar.gz"
      else
        (cd release && zip -qrX "${NAME}.zip" "${NAME}")
