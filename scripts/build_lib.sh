...
if [ -z "${VERSION:-}" ]; then
  VERSION=$(git describe --always --dirty --tags --match='v*' --abbrev=14 || echo "GitNotFound")
fi
...
GO_LDFLAGS=(${GO_LDFLAGS:-} "-X=${VERSIONSHA_SYMBOL}=${GIT_SHA}" "-X=${VERSION_SYMBOL}=${VERSION}")
