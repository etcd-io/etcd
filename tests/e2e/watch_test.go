...
func TestWatchWithLowPermissionUser(t *testing.T) {
	...
	ec := epc.Etcdctl()
	_, err = ec.UserAdd("root", "rootPassword")
	require.NoError(t, err)
	_, err = ec.UserGrantRole("root", "root")
	require.NoError(t, err)
	_, err = ec.RoleAdd("test")
	require.NoError(t, err)
	// Grant read permission on the EXACT single key "foo" only (RangeEnd == "").
	_, err = ec.RoleGrantPermission("test", "foo", "", clientv3.PermissionType(clientv3.PermRead))
	require.NoError(t, err)
	_, err = ec.UserAdd("test", "testPassword")
	require.NoError(t, err)
	_, err = ec.UserGrantRole("test", "test")
	require.NoError(t, err)

	t.Log("Enable auth")
	require.NoError(t, ec.AuthEnable())
