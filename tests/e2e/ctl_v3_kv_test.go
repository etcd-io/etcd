...
func getFormatTest(cx ctlCtx) {
	...
	for i, tt := range tests {
		...
		if err := testutils.SpawnWithExpectWithEnv(cmdArgs, cx.envMap, tt.wstr); err != nil {
...
func getKeysOnlyTest(cx ctlCtx) {
	...
	if err := testutils.SpawnWithExpectWithEnv(cmdArgs, cx.envMap, "key"); err != nil {
		cx.t.Fatal(err)
	}
	...
	lines, err := testutils.SpawnWithExpectLines(ctx, cmdArgs, cx.envMap, "key")
...
func getCountOnlyTest(cx ctlCtx) {
	...
	if err := testutils.SpawnWithExpects(cmdArgs, cx.envMap, "\"Count\" : 0"); err != nil {
		cx.t.Fatal(err)
	}
	if err := ctlV3Put(cx, "key", "val", ""); err != nil {
		cx.t.Fatal(err)
	...
	cmdArgs = append(cx.PrefixArgs(), []string{"get", "--count-only", "key", "--prefix", "--write-out=fields"}...)
	if err := testutils.SpawnWithExpects(cmdArgs, cx.envMap, "\"Count\" : 1"); err != nil {
		cx.t.Fatal(err)
	}
	...
	cmdArgs = append(cx.PrefixArgs(), []string{"get", "--count-only", "key", "--prefix", "--write-out=fields"}...)
	if err := testutils.SpawnWithExpects(cmdArgs, cx.envMap, "\"Count\" : 2"); err != nil {
		cx.t.Fatal(err)
	}
	...
	cmdArgs = append(cx.PrefixArgs(), []string{"get", "--count-only", "key", "--prefix", "--write-out=fields"}...)
	if err := testutils.SpawnWithExpects(cmdArgs, cx.envMap, "\"Count\" : 3"); err != nil {
		cx.t.Fatal(err)
	}
	...
	lines, err := testutils.SpawnWithExpectLines(ctx, cmdArgs, cx.envMap, "\"Count\"")
	require.NoError(cx.t, err)
...
func ctlV3Put(cx ctlCtx, key, value, leaseID string, flags ...string) error {
	...
	return testutils.SpawnWithExpectWithEnv(cmdArgs, cx.envMap, "OK")
}
...
func ctlV3GetWithErr(cx ctlCtx, args []string, errs []string) error {
	...
	return testutils.SpawnWithExpects(cmdArgs, cx.envMap, errs...)
}

func ctlV3Del(cx ctlCtx, args []string, num int) error {
	cmdArgs := append(cx.PrefixArgs(), "del")
	cmdArgs = append(cmdArgs, args...)
	return testutils.SpawnWithExpects(cmdArgs, cx.envMap, fmt.Sprintf("%d", num))
}
