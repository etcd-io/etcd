...
// kv represents a key-value pair for testing
type kv struct {
	key, val string
}

// ctlV3Put puts a key-value pair into etcd using etcdctl
...
// ctlV3Get gets values for the given keys using etcdctl
...
// ctlV3Del deletes keys using etcdctl and expects the given number of deletions
func ctlV3Del(cx ctlCtx, args []string, num int) error {
	cmdArgs := append(cx.PrefixArgs(), "del")
	cmdArgs = append(cmdArgs, args...)

	return e2e.SpawnWithExpectWithEnv(cmdArgs, cx.envMap, expect.ExpectedResponse{Value: fmt.Sprintf("%d", num)})
}
