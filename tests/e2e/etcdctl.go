...
func (ctl *Etcdctl) flags() map[string]string {
	...
	if ctl.v2 {
		if ctl.connType == clientTLS {
			fmap["ca-file"] = "../../integration/fixtures/ca.crt"
			fmap["cert-file"] = "../../integration/fixtures/ca.crt"
			fmap["key-file"] = "../../integration/fixtures/server.key.insecure"
		}
	} else {
		if ctl.connType == clientTLS {
			if ctl.isAutoTLS {
				...
				fmap["cacert"] = "../../integration/fixtures/ca.crt"
				fmap["cert"] = "../../integration/fixtures/server.crt"
				fmap["key"] = "../../integration/fixtures/server.key.insecure"
			}
		}
	}
...
func (ctl *Etcdctl) env() []string {
	if ctl.v2 {
		return []string{"ETCDCTL_API=2"}
	}
	return []string{"ETCDCTL_API=3"}
}
