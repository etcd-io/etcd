...
func runTraffic(ctx context.Context, lg *zap.Logger, tf traffic.Traffic, hosts []string, baseTime time.Time, duration time.Duration) ([]report.ClientReport, error) {
	...
	g.Go(func() error {
		...
		trafficReports := simulateTraffic(ctx, tf, hosts, trafficSet, duration)
...
func simulateTraffic(ctx context.Context, tf traffic.Traffic, hosts []string, clientSet *client.ClientSet, duration time.Duration) []report.ClientReport {
