...
func collectClusterWatchEvents(ctx context.Context, t *testing.T, clus *e2e.EtcdProcessCluster, maxRevisionChan <-chan int64, cfg watchConfig, baseTime time.Time, ids identity.Provider) []model.ClientReport {
	mux := sync.Mutex{}
	var wg sync.WaitGroup
	reports := make([]model.ClientReport, len(clus.Procs))
...
func validateGotAtLeastOneProgressNotify(t *testing.T, reports []model.ClientReport, expectProgressNotify bool) {
