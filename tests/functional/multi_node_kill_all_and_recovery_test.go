...
func TestMultiNodeKillAllAndRecoveryWithStandbys(t *testing.T) {
	...
	resp, _ := tests.Put("http://localhost:7001/v2/admin/config", "application/json", bytes.NewBufferString(`{"activeSize":3, "promoteDelay":1800}`))
	...
	// Verify that the standby node is now two peers.
	...
	// Verify that we have two standby.
	result, err = c.Get("_etcd/standbys", false, true)
	assert.NoError(t, err)
	assert.Equal(t, len(result.Node.Nodes), 2)
}
