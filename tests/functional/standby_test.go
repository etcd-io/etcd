...
func TestStandby(t *testing.T) {
	...
	// Verify that we just have five machines.
	...
	// Verify that we now have three peers.
	result, err = c.Get("_etcd/machines", false, true)
	assert.NoError(t, err)
	...
	resp, _ = tests.Put("http://localhost:7001/v2/admin/config", "application/json", bytes.NewBufferString(`{"activeSize":11, "syncClusterInterval":1}`))
	if !assert.Equal(t, resp.StatusCode, 200) {
		t.FailNow()
	}
	...
	// Verify that exactly four machines are in the cluster.
	result, err = c.Get("_etcd/machines", false, true)
	assert.NoError(t, err)
	assert.Equal(t, len(result.Node.Nodes), 11)
}
