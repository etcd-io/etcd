...
func TestCARotation(t *testing.T) {
	...
	// Create CAReloader for dynamic CA updates
	caReloader, err := tlsutil.NewCAReloader([]string{filepath.Join(certDir, "ca.pem")}, zaptest.NewLogger(t))
	require.NoError(t, err)
	caReloader.WithInterval(100 * time.Millisecond) // Fast reload for testing
	caReloader.Start()
	t.Cleanup(func() {
		caReloader.Stop()
	})
