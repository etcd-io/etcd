...
func (b *bridge) serveConn(bc *bridgeConn) {
	...
	wg.Add(2)
	go func() {
		ioCopy(bc.out, bc.in, b.blackholec)
		bc.close()
		wg.Done()
	}()
	go func() {
		ioCopy(bc.in, bc.out, b.blackholec)
		bc.close()
		wg.Done()
	}()
	wg.Wait()
...
// https://github.com/golang/go/blob/master/src/io/io.go copyBuffer
func ioCopy(dst io.Writer, src io.Reader, blackholec chan struct{}) (err error) {
