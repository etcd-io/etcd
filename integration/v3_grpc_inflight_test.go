...
func TestV3KVInflightRangeRequests(t *testing.T) {
	...
	for i := 0; i < reqN; i++ {
		go func() {
			...
			if err != nil {
				terr, ok := err.(transport.ConnectionError)
				if rpctypes.ErrorDesc(err) != context.Canceled.Error() && (ok && terr != transport.ErrConnClosing) {
					t.Fatalf("inflight request should be canceld with %q or %q, got %q", context.Canceled.Error(), transport.ErrConnClosing.Error(), err.Error())
				}
			}
		}()
	}
