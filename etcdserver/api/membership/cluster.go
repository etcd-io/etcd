...
func mustDetectDowngrade(lg *zap.Logger, cv *semver.Version) {
	...
	if cv != nil && lv.LessThan(*cv) {
		if IsVersionChangable(cv, lv) {
			if lg != nil {
				lg.Info(
					"cluster is downgrading to current server version from determined cluster version",
					zap.String("current-server-version", version.Version),
					zap.String("determined-cluster-version", version.Cluster(cv.String())),
				)
			} else {
				plog.Infof("cluster is downgrading to current server version: %s from determined cluster version: %s).", version.Version, version.Cluster(cv.String()))
			}
		} else {
			if lg != nil {
				lg.Fatal(
