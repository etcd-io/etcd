...
func TestLeaseKeepAliveNewAfterClose(t *testing.T) {
	defer testutil.AfterTest(t)
	...
	donec := make(chan struct{})
	go func() {
		if _, err := le.KeepAlive(context.TODO(), leaseID); err != rpctypes.ErrConnClosed {
			t.Fatalf("expected %v, got %v", rpctypes.ErrConnClosed, err)
		}
		close(donec)
	}()
	select {
	case <-time.After(3 * time.Second):
		t.Fatal("le.KeepAlive took too long")
	case <-donec:
	}
}
