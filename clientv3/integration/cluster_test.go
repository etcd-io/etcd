...
func TestMaxLearnerInCluster(t *testing.T) {
	...
	resp1, err := clus.Client(0).MemberAdd(context.Background(), []string{"http://127.0.0.1:1234"}, true)
	if err != nil {
	...
	_, err = clus.Client(0).MemberAdd(context.Background(), []string{"http://127.0.0.1:2345"}, true)
	if err == nil {
	...
	_, err = clus.Client(0).MemberAdd(context.Background(), []string{"http://127.0.0.1:3456"}, false)
	if err != nil {
