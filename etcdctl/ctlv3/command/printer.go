...
func (s *simplePrinter) Del(resp v3.DeleteResponse) {
	...
	for _, kv := range resp.PrevKvs {
		printKV(s.isHex, s.valueOnly, false, kv)
	}
}

func (s *simplePrinter) Get(resp v3.GetResponse) {
	for _, kv := range resp.Kvs {
		printKV(s.isHex, s.valueOnly, s.lease, kv)
	}
}
...
func (s *simplePrinter) Put(r v3.PutResponse) {
	fmt.Println("OK")
	if r.PrevKv != nil {
		printKV(s.isHex, s.valueOnly, false, r.PrevKv)
	}
}
...
func (s *simplePrinter) Watch(resp v3.WatchResponse) {
	for _, e := range resp.Events {
		fmt.Println(e.Type)
		if e.PrevKv != nil {
			printKV(s.isHex, s.valueOnly, false, e.PrevKv)
		}
		printKV(s.isHex, s.valueOnly, false, e.Kv)
	}
}
