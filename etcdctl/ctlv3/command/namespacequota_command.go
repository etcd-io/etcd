...
func namespaceQuotaSetCommandFunc(cmd *cobra.Command, args []string) {
	if len(args) != 2 {
		ExitWithError(ExitBadArgs, fmt.Errorf("namespace quota command needs a key and atleast a byte quota or key quota value"))
	}
	...
	if len(key) == 0 || len(args[1]) == 0 {
		ExitWithError(ExitBadArgs, fmt.Errorf("namespace quota command needs a key AND, a byte quota OR key quota value"))
	}
	...
	for _, value := range values {
		...
		if currQuota[0] == "b" {
			...
			if err != nil {
				ExitWithError(ExitBadArgs, fmt.Errorf("bad byteq quota count (%v)", err))
			}
		} else if currQuota[0] == "k" {
			keyQuota, err = strconv.ParseUint(currQuota[1], 10, 64)
			if err != nil {
				ExitWithError(ExitBadArgs, fmt.Errorf("bad key quota count (%v)", err))
			}
		} else {
			ExitWithError(ExitBadArgs, fmt.Errorf("unrecognizable quota type (%v)", currQuota[0]))
		}
	}
	...
	cancel()
	if err != nil {
		ExitWithError(ExitError, fmt.Errorf("failed to set namespace quota (%v)", err))
	}
...
func namespaceQuotaDeleteCommandFunc(cmd *cobra.Command, args []string) {
	if len(args) != 1 {
		ExitWithError(ExitBadArgs, fmt.Errorf("namespace quota delete command needs one non empty key"))
	}
	...
	cancel()
	if err != nil {
		ExitWithError(ExitError, fmt.Errorf("failed to delete namespace quota (%v)", err))
	}
...
func namespaceQuotaGetCommandFunc(cmd *cobra.Command, args []string) {
	if len(args) != 1 {
		ExitWithError(ExitBadArgs, fmt.Errorf("namespace quota get command needs one non empty key"))
	}
	...
	cancel()
	if err != nil {
		ExitWithError(ExitError, fmt.Errorf("failed to get namespace quota (%v)", err))
	}
...
func namespaceQuotaListCommandFunc(cmd *cobra.Command, args []string) {
	...
	cancel()
	if err != nil {
		ExitWithError(ExitError, fmt.Errorf("failed to list namespace quota (%v)", err))
	}
