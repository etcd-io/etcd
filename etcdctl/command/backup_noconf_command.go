...
func NewBackupNoConfCommand() cli.Command {
	return cli.Command{
		Name:  "backup-noconf",
		...
		Action: handleBackupNoConf,
	}
}

// handleBackup handles a request that intends to do a backup.
func handleBackupNoConf(c *cli.Context) {
	...
	if snapshot != nil {
		...
		snapshot, err := purgeConfInSnap(snapshot)
		if err != nil {
			log.Fatal(err)
		}
	...
	idgen := idutil.NewGenerator(uint8(time.Now().UnixNano()), time.Now())
	...
	defer neww.Close()
	if err := neww.Save(state, purgeConfInEnts(ents)); err != nil {
		log.Fatal(err)
	}
...
func purgeConfInSnap(snap *raftpb.Snapshot) (*raftpb.Snapshot, error) {
	st := store.New()
	err := st.Recovery(snap.Data)
	if err != nil {
		return nil, err
	}
	st.Delete("/0", true, true)
	d, err := st.Save()
	if err != nil {
		return nil, err
	}
	snap.Data = d
	snap.Metadata.ConfState = raftpb.ConfState{Nodes: make([]uint64, 0)}
	return snap, nil
}

func purgeConfInEnts(ents []raftpb.Entry) []raftpb.Entry {
	after := make([]raftpb.Entry, 0)
	for _, e := range ents {
		switch e.Type {
		...
		default:
			log.Fatalf("unexpected entry type: %d", e.Type)
		}
