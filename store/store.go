...
type Store interface {
	...
	Watch(prefix string, recursive bool, stream bool, sinceIndex uint64) (*watcher, error)
	Save() ([]byte, error)
...
func (s *store) Watch(prefix string, recursive bool, stream bool, sinceIndex uint64) (*watcher, error) {
	...
	if sinceIndex == 0 {
		w, err = s.WatcherHub.watch(prefix, recursive, stream, nextIndex)

	} else {
		w, err = s.WatcherHub.watch(prefix, recursive, stream, sinceIndex)
	}
