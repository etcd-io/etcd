...
```sh
curl -L http://127.0.0.1:7001/v2/admin/config -XPUT -d '{"activeSize":3, "removeDelay":1800, "syncInterval":10}'
```
...
`activeSize` is the maximum number of peers that can participate the consensus protocol in the cluster.

The size of cluster is controlled to be around this number. If it is higher than active size, the leader will remove some node. On the other hand, standby machines will try to join the cluster if they find out that the cluster size is lower than active size.

`removeDelay` indicates the minimum length of delay that has been observed before removing a node.

`syncInterval` is the interval between synchronization to the cluster in standby mode.

The value will come into effect for a standby machine after its next synchronization.
