...
A network partition is similar to a minor followers failure or a leader failure. A network partition divides the etcd cluster into two parts; one with a member majority and the other with a member minority. The majority side becomes the available cluster and the minority side is unavailable; there is no “split-brain” in etcd.  ( no “split-brain” is possible because etcd cluster members are explicitly added/removed, with each such change approved by the current majority, and thus allows cluster members can determine majority. ) 
