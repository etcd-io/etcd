...
func Start(config *config.Config) (s *server.Server, stop func(), wait func(), running bool, e error) {
	...
	// Create etcd server
	s = server.New(config.Name, config.SanitizedAddr, ps, registry, store, &mb)
	...
	// Server could receive requests from now on.
	log.Infof("peer server [name %s, listen on %s, advertised url %s]", ps.Config.Name, config.Peer.SanitizedBindAddr, ps.Config.URL)
	psListener := server.NewListener(psConfig.Scheme, config.Peer.SanitizedBindAddr, peerTLSConfig)
	log.Infof("etcd server [name %s, listen on %s, advertised url %s]", s.Name, config.SanitizedBindAddr, s.URL())
	sListener := server.NewListener(config.EtcdTLSInfo().Scheme(), config.SanitizedBindAddr, etcdTLSConfig)
	...
	// Peer server may be stopped due to receive remove command.
	// This goroutine helps to close all listeners.
	go func() {
		select {
		case <-ps.RemoveNotify():
		case <-stopNotify:
		}
		ps.Stop()
		psListener.Close()
