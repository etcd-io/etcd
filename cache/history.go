...
// Any implementation must be goroutine‑safe.
type History interface {
	// Append stores one event in chronological order.
	Append(ev *clientv3.Event)
	// GetSince returns all events with ModRevision >= rev that match pred (nil predicate matches all).
	GetSince(rev int64, pred KeyPredicate) []*clientv3.Event
