...
import (
	...
	"go.etcd.io/etcd/namespacequota/namespacequotapb"
	"go.uber.org/zap"
	"math"
	"sync"
	"time"
...
func (nq *NamespaceQuota) persistTo(b backend.Backend) {
	...
	if err != nil {
		panic("failed to marshal namespace quota proto item: " + err.Error())
	}

	b.BatchTx().Lock()
	b.BatchTx().UnsafePut(namespaceQuotaBucketName, []byte(key), val)
	b.BatchTx().Unlock()
}
...
func newNamespaceQuotaManager(lg *zap.Logger, b backend.Backend, cfg NamespaceQuotaManagerConfig) *namespaceQuotaManager {
	...
	nqm.initAndRecover()
	if lg != nil {
		lg.Info("namespace quota manager config", zap.Int64("enforcement status", int64(nqm.namespaceQuotaEnforcement)))
	}
...
func (nqm *namespaceQuotaManager) initAndRecover() {
	...
	tx.UnsafeCreateBucket(namespaceQuotaBucketName)
	_, quotas := tx.UnsafeRange(namespaceQuotaBucketName, []byte{0}, []byte{0xff}, -1)
	// TODO: copy vs and do decoding outside tx lock if lock contention becomes an issue.
	for i := range quotas {
		var nqmpb namespacequotapb.NamespaceQuota
		err := nqmpb.Unmarshal(quotas[i])
		if err != nil {
			tx.Unlock()
			panic("failed to unmarshal namespace quota proto item: " + err.Error())
		}
		key := []byte(nqmpb.Key)
		nqm.namespaceQuotaTree.Put(&NamespaceQuota{
			Key:            key,
			QuotaKeyCount:  nqmpb.QuotaKeyCount,
			QuotaByteCount: nqmpb.QuotaByteCount,
			UsageKeyCount:  nqmpb.UsageKeyCount,
			UsageByteCount: nqmpb.UsageByteCount,
		})
	}
	tx.Unlock()
	nqm.b.ForceCommit()
